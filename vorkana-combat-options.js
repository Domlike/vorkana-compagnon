...
(function () {
  ...
  function installPlayer() {
    ...
    for(const name of ['crRenderInitiative','crRollInitiative']){const old=window[name];window[name]=function(...args){const initiative=P.combat.initiative,step=initiative.step,dice=initiative.dice;initiative.step=step-healthActionPenalty()+actorConditions().action+mods(key()).action;if(typeof REF==='undefined'||REF.id!=='barbak')initiative.step=Math.max(1,initiative.step);initiative.dice=edStepDiceLabel(edStepSpec(initiative.step));try{return old(...args);}finally{initiative.step=step;initiative.dice=dice;}};}
